...
namespace minihito.persistence
{
    ...
    internal class AlumnadoPersistence
    {
        ...
        List<Alumnado> ListaAlumnos { get; set; }

        public AlumnadoPersistence()
        {
            table = new DataTable();
            ListaAlumnos = new List<Alumnado>();
        }
        ...
        {
            String sql = $"INSERT INTO aceptasreto.alumnado VALUES (null,'{alumno.Nombre}','{alumno.Apellidos}','{alumno.Especialidad}',null)";
            DBBroker.obtenerAgente().modificar(sql);
        }
        public void BorrarPersona(Alumnado alumno)
        {
            String sql = $"DELETE FROM aceptasreto.alumnado WHERE idAlumnado = {alumno.Id};";
            DBBroker.obtenerAgente().modificar(sql);
        }
        public void ModificarPersona(Alumnado alumno)
        {
        ...
        public List<Alumnado> LeerAlumnos()
        {
            ...
            List<Object> aux = DBBroker.obtenerAgente().leer(
                "SELECT idAlumnado, nombre, apellidos, especialidad, grupo FROM aceptasreto.alumnado ORDER BY nombre;"
            );
        ...
        public List<Alumnado> LeerAlumnosSinGrupo()
        {
            List<Alumnado> lista = new List<Alumnado>();
            List<Object> aux = DBBroker.obtenerAgente().leer(
                "SELECT idAlumnado, nombre, apellidos, especialidad, grupo FROM aceptasreto.alumnado WHERE grupo IS NULL ORDER BY nombre;"
            );
        ...
        public List<Alumnado> LeerAlumnosPorGrupo(int idGrupo)
        {
            List<Alumnado> lista = new List<Alumnado>();
            List<Object> aux = DBBroker.obtenerAgente().leer(
                $"SELECT idAlumnado, nombre, apellidos, especialidad, grupo FROM aceptasreto.alumnado WHERE grupo = {idGrupo} ORDER BY nombre;"
            );
        ...
        {
            String sql = $"INSERT INTO aceptasreto.alumnado (nombre, apellidos, especialidad, grupo) " +
                        $"VALUES ('{alumno.Nombre}', '{alumno.Apellidos}', {alumno.Especialidad}, NULL);";
            DBBroker.obtenerAgente().modificar(sql);
        }
