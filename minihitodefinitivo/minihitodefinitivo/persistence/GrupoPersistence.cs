...
namespace minihitodefinitivo.persistence
{
    internal class GrupoPersistence
    {
        public List<Grupo> LeerGrupos()
        {
            ...
            List<Object> aux = DBBroker.obtenerAgente().leer("SELECT idGrupo, nombre FROM aceptasreto.grupo ORDER BY nombre;");
        ...
        {
            List<Object> aux = DBBroker.obtenerAgente().leer("SELECT MAX(idGrupo) FROM aceptasreto.grupo;");
            foreach (List<Object> c in aux)
            {
